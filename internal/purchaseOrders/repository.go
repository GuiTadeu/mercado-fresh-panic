...
type PurchaseOrdersRepository interface {
	...
	ExistsPurchaseOrderId(id uint64) (bool, error)
}
...
func (r *purchaseOrdersRepository) ExistsPurchaseOrderId(id uint64) (bool, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&myPurchaseOrder.Id,
			&myPurchaseOrder.OrderNumber,
			&myPurchaseOrder.OrderDate,
			&myPurchaseOrder.TrackingCode,
			&myPurchaseOrder.BuyerId,
			&myPurchaseOrder.OrderStatusId,
			&myPurchaseOrder.ProductRecordId,
		)
		if err != nil {
			return false, err
		}
